...
solToCent2 = do
  ...
  runBefore 1 isVertical
  addAnimAt (-400, 300) (textBox "Going to be vertical!!")
  runTill isVertical

  img .= p -- restoring image
  addAnimAt (-400, 300) (textBox "Now vertical!!")
  ...
  img .= p
  addAnimAt (-400, 300) (textBox "No force!!")
  theBodies.traverse.bmass *= 1/0.9

  runFor 4
--  runTill (clockExceeded 6.02)
  --change .= fromInternal gravitationField
  --runTill (clockExceeded 10)
...
--main = initPhysical solarSystem >>= solarToCentri
--main = runSimulate solToCent (initPhy solarSystem)
main = runSimSets solToCent3 (initPhy solarSystem)
